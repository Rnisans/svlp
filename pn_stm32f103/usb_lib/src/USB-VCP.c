...
void USB_MANAGEMENT()	{
		if(USB_RECEIVED_EP0)	{												//If a packet has been received on endpoint 0..
			...
			else if (p_mem_8[0] == 0x21 && p_mem_8[1] == 0x20) { // SET_LINE_CODING
			            EN_RX_EP0();
			            while(USB_RECEIVED_EP0 == 0) {};
			            USB_RECEIVED_EP0 = 0;

			            // Обновление параметров порта
...
void USB_LP_CAN1_RX0_IRQHandler(void) {
	...
	if(USB_ISTR & 0x8000)	{						//If a transmission has been completed correctly..
		...
		if(USB_ENP1R & 0x0080)	{					//If the transmission is ГЁ EP1 IN..
